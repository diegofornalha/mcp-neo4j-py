...
if docker ps | grep -q terminal-neo4j; then
    ...
    # Testar conexão
    if docker exec terminal-neo4j cypher-shell -u neo4j -p "password" "RETURN 1" &>/dev/null; then
...
echo -e "\n5️⃣ Dados no Neo4j:"
MEMORY_COUNT=$(docker exec terminal-neo4j cypher-shell -u neo4j -p "password" --format plain "MATCH (m:Memory) RETURN COUNT(m) as count" 2>/dev/null | grep -o '[0-9]*' | head -1)
